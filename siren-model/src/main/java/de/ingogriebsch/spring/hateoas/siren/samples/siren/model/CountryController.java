...
/**
 * A {@link RestController} implementation that controls the access to the available country resources.
 * <p>
 ...
 */
@RequestMapping(CountryController.BASE_PATH)
@RequiredArgsConstructor
@RestController
class CountryController {

    static final String BASE_PATH = "/countries";
    ...
    @GetMapping
    ResponseEntity<PagedModel<RepresentationModel<?>>> findAll(Pageable pageable) {
        return ok(countryModelAssembler.toPagedModel(countryService.findAll(pageable)));
    }

    @GetMapping(FIND_ONE_PATH)
    ResponseEntity<? extends RepresentationModel<?>> findOne(@PathVariable String id) {
        return countryService.findOne(id).map(c -> ok(countryModelAssembler.toModel(c))).orElse(notFound().build());
    }
}
